...
test('it serializes GitHub thread requests', function(assert) {
  ...
  Ember.$.mockjax({
    ...
    dataType: 'json',
    responseText: _this.response
  });

  Ember.run(function() {
    _this.store().find('github-thread', 'fake-thread-1').then(function(result) {
      var subscription = JSON.parse(JSON.stringify(result.get('githubSubscription')));
      var threadComment = JSON.parse(JSON.stringify(result.get('githubThreadComment')));
      ...
      assert.equal(subscription.content.subscribed, false);
      assert.equal(subscription.content.ignored, false);
      assert.equal(subscription.content.reason, null);
      assert.equal(subscription.content.createdAt, null);
      assert.equal(subscription.content.url, null);
      assert.equal(subscription.content.threadUrl, null);
      assert.equal(threadComment.content.url, null);
      assert.equal(threadComment.content.htmlUrl, null);
      assert.equal(threadComment.content.userLogin, null);
      assert.equal(threadComment.content.userId, null);
      assert.equal(threadComment.content.userAvatarUrl, null);
      assert.equal(threadComment.content.userHtmlUrl, null);
      assert.equal(threadComment.content.createdAt, null);
      assert.equal(threadComment.content.updatedAt, null);
    });
  });
});

test('it handles null values for the latest comment url gracefully', function(assert) {
  assert.expect(13);
  var _this = this;
  this.response.subject.latest_comment_url = null;  // eslint-disable-line camelcase
  ...
  Ember.run(function() {
    _this.store().find('github-thread', 'fake-thread-1').then(function(result) {
      ...
      assert.equal(result.get('subjectUrl'), "https://api.github.com/repos/spf13/hugo/issues/2184");
      assert.equal(result.get('subjectType'), "Issue");
      assert.equal(result.get('subjectLatestCommentUrl'), null);
      assert.equal(result.get('subscriptionUrl'), "https://api.github.com/notifications/threads/146393869/subscription");
      assert.deepEqual(result.get('updatedAt'), new Date("2016-06-06T15:52:17Z"));
      assert.equal(subscription.content.subscribed, false);
      assert.equal(subscription.content.ignored, false);
      ...
      assert.equal(subscription.content.url, null);
      assert.equal(subscription.content.threadUrl, null);
      assert.equal(threadComment.content, null);
    });
  });
});

test('it handles null values for the subscription url gracefully', function(assert) {
  assert.expect(15);
  var _this = this;
  this.response.subscription_url = null;  // eslint-disable-line camelcase
  ...
  Ember.run(function() {
    _this.store().find('github-thread', 'fake-thread-1').then(function(result) {
      ...
      assert.equal(result.get('subjectType'), "Issue");
      assert.equal(result.get('subjectLatestCommentUrl'), "https://api.github.com/repos/spf13/hugo/issues/2184");
      assert.equal(result.get('subscriptionUrl'), null);
      assert.deepEqual(result.get('updatedAt'), new Date("2016-06-06T15:52:17Z"));
      assert.equal(threadComment.content.url, null);
      assert.equal(threadComment.content.htmlUrl, null);
      assert.equal(threadComment.content.userLogin, null);
      ...
      assert.equal(threadComment.content.createdAt, null);
      assert.equal(threadComment.content.updatedAt, null);
      assert.equal(subscription.content, null);
    });
  });
});
